...
function buildPopupJs() {
    return '(function() {\n'
        ...
        + '    };\n'
        + '    /* 팝업에서 직접 fetch 대신 부모 창(admin.html)의 함수를 호출 — RLS/인증 문제 회피 */\n'
        + '    var opener = window.opener || window.parent;\n'
        + '    if (opener && typeof opener.saveReceiptFromPopup === "function") {\n'
        + '      opener.saveReceiptFromPopup(payload, function(savedId) {\n'
        + '        if (savedId) {\n'
        + '          window._savedReceiptId = savedId;\n'
        + '          document.getElementById("btnS").textContent = "\u2705 저장됨 (계약서 탭에서 발송)";\n'
        + '          document.getElementById("btnS").style.background = "#047857";\n'
        + '          document.getElementById("btnSend").style.display = "";\n'
        + '        } else { alert("저장에 실패했습니다. 다시 시도해 주세요."); }\n'
        + '      });\n'
        + '    } else { alert("부모 창과의 연결이 끊겼습니다. 팝업을 닫고 다시 시도해 주세요."); }\n'
        + '  });\n'
